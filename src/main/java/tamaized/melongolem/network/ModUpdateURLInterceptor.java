...
public class ModUpdateURLInterceptor {
	...
	public ModUpdateURLInterceptor() {
		ModList.get().getModFileById(MelonMod.MODID).getMods().forEach(info -> {
			if (info instanceof ModInfo mod) {
				...
				if (ModInfo_updateJSONURL != null) {
					try {
						ModInfo_updateJSONURL.invokeExact(mod, Optional.of(URI.create("https://gh.tamaized.com/Tamaized/MelonGolem/update.json?m=%s&l=%s&v=%s".formatted(
