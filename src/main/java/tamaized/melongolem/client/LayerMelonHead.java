...
public class LayerMelonHead extends RenderLayer<MelonGolemRenderState, SnowGolemModel> {
	...
	public void submit(PoseStack stack, SubmitNodeCollector buffer, int light, MelonGolemRenderState entity, float yawHead, float pitch) {
		ItemStack itemStack = entity.headStack;
		if (!entity.isInvisible || !itemStack.isEmpty()) {
			...
			if (itemStack.is(ItemTags.SIGNS) && itemStack.getItem() instanceof StandingAndWallBlockItem item && entity.signTileEntity != null) {
				...
				entity.signTileEntity.frontText = new SignText(signText, signText, entity.textColor, entity.isTextGlowing);
				entity.signTileEntity.setBlockState(item.wallBlock.defaultBlockState());
				BlockEntityRenderState teState = Minecraft.getInstance().getBlockEntityRenderDispatcher().tryExtractRenderState(entity.signTileEntity, entity.partialTick, null, null);

				if (teState instanceof SignRenderState signState) {
					Objects.requireNonNull(Minecraft.getInstance().getBlockEntityRenderDispatcher().getRenderer(entity.signTileEntity)).submit(signState, stack, buffer, Minecraft.getInstance().gameRenderer.getGameRenderState().levelRenderState.cameraRenderState);
				}
			} else
