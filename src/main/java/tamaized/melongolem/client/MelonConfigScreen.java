...
public class MelonConfigScreen extends Screen {
	...
	protected void init() {
		super.init();
		float sw = Minecraft.getInstance().mainWindow.getScaledWidth();
		int w = 396;
		float x = (sw - w);
		x -= x / 2F;
		addButton(new GuiButtonExt((int) x, Minecraft.getInstance().mainWindow.getScaledHeight() - 25, w, 20, "Close", button -> Minecraft.getInstance().displayGuiScreen(parent)));
		int ix = font.getStringWidth(TEXT_DONATOR_COLOR) + 10;
		addButton(input = new TextFieldWidget(font, ix, 25, Minecraft.getInstance().mainWindow.getScaledWidth() - ix - 15, 20, ""));
	...
	public void render(int p_render_1_, int p_render_2_, float p_render_3_) {
		renderBackground();
		drawCenteredString(font, "Melon Golem Config", (int) (Minecraft.getInstance().mainWindow.getScaledWidth() / 2F), 5, 0xFFFFFFFF);
